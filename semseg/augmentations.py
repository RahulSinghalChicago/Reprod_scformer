...
import cv2
import numpy as np
from torchvision.transforms.transforms import PILToTensor, ToPILImage, ToTensor
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        Resize(size),
        RandomResizedCrop(size, scale=(0.75, 1.25), seg_fill=seg_fill),
        RandomHorizontalFlip(p=0.5),
        RandomVerticalFlip(p=0.5),
        RandomRotation(degrees=359, p=0.3, seg_fill=seg_fill),
        RandomImageEnhance(),
        ...
        # Kvasir masked
        #Normalize((0.80428439, 0.475362  , 0.34986879), (0.1220109 , 0.14156408, 0.13634158))
        # for Kvasir unmasked
        #Normalize((0.55464893, 0.32034812, 0.23539365), (0.30882672, 0.21766038, 0.18177935))
        ##Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
    ])
