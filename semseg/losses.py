...
class Dice(nn.Module):
    ...
    def _forward(self, preds: Tensor, labels: Tensor) -> Tensor:
        ...
        #print(labels.shape, preds.shape, tp.shape)
        #print(labels[0], preds[0], tp)
...
class DCE(nn.Module):
    # added delta (for Dice) to init parameters for CE
    def __init__(self, delta: float = 0.5, ignore_label: int = 255, weight: Tensor = None, aux_weights: list = [1, 0.4, 0.4]) -> None:
        super().__init__()
        self.delta = delta
        self.aux_weights = aux_weights
        #print(type(ignore_label))
        #print("DCE: ",self.delta, ignore_label, weight, aux_weights)
        print("weight: ",weight)
        self.criterion = nn.CrossEntropyLoss(weight=weight, ignore_index=ignore_label)

    def _forward(self, preds: Tensor, labels: Tensor) -> Tensor:
        #print("_fwd: ", preds.shape, labels.shape)
        #print(labels[0])
        ...
        dice_score = dice_score / num_classes
        #print("_forward")
        #print("_fwd: ", dice_score.mean(), CE)

        return dice_score.mean() + CE

    def forward(self, preds, labels: Tensor) -> Tensor:
        if isinstance(preds, tuple):
            return sum([w * self._forward(pred, labels) for (pred, w) in zip(preds, self.aux_weights)])
        #print("fwd: ", preds.shape, labels.shape)
        return self._forward(preds, labels)
...
class DiceLoss(nn.Module):
    ...
    Return:
        diceloss
    """
    def __init__(self, delta: float = 0.5, weight = None):
...
__all__ = ['CrossEntropy', 'OhemCrossEntropy', 'Dice', 'DCE', 'DiceLoss']
