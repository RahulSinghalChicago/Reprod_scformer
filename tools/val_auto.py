...
def main(cfg):
    ...
    model_path_list = eval_cfg['MODEL_PATH']

    model_cfg =  cfg['MODEL']
    backbone_list = model_cfg['BACKBONE']
    length_backbone = len(backbone_list)

    model_list = []
    for _, model_name in enumerate(model_cfg['NAME']):
        model_list = model_list + [model_name]*length_backbone

    backbone_list = backbone_list*len(model_cfg['NAME'])

    for n in range(len(backbone_list)):
        model_path_loc = model_path_list[n]
        model_name = model_list[n]
        backbone_name = backbone_list[n]
        model_path = Path(model_path_loc)
        if not model_path.exists():
            print("Error with path: ", model_path_loc)
            break
        
        print(f"Evaluating {model_path}...")

        model = eval(model_name)(backbone_name, dataset.n_classes)
