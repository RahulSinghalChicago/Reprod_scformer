...
from semseg.utils.utils import fix_seeds, setup_cudnn, cleanup_ddp, setup_ddp, get_model_size
...
def main(cfg, gpu, save_dir):
    ...
    model_list = model_cfg['NAME']
    backbone_list = model_cfg['BACKBONE']
    pretrain_list = model_cfg['PRETRAINED']

    for bidx, (backbone_name, pretrain_path) in enumerate(zip(backbone_list, pretrain_list)):        
        for _, model_name in enumerate(model_list):
            fix_seeds(42)
            best_mIoU = 0.0
            start = time.time()

            model = eval(model_name)(backbone_name, trainset.n_classes)
            model.init_pretrained(pretrain_path)    
            print("Model Size: ", get_model_size(model), " for ", backbone_name, " ", model_name)
            ...
            for epoch in range(epochs):
                ...
                if (epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 or (epoch+1) == epochs:
                    ...
                    if miou > best_mIoU:
                        best_mIoU = miou
                        torch.save(model.module.state_dict() if train_cfg['DDP'] else model.state_dict(), save_dir / f"{model_name}_{backbone_name}_{dataset_cfg['NAME']}.pth")
